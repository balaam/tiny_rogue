...
namespace game
{
    ...
    public class SlimeSystem : ComponentSystem
    {
        protected override void OnUpdate()
        {
            ...
            Entities.ForEach((Entity creature, ref WorldCoord coord, ref HealthPoints hp) =>
            {
                ...
                int dmg = 0;
                Entities.WithAll<Slime>().ForEach((Entity trap, ref WorldCoord trapCoord) =>
                {
                    if (creaturePos.x == trapCoord.x && creaturePos.y == trapCoord.y)
                    {
                        if(EntityManager.HasComponent(creature, typeof(Player)))
                        {
                            ...
                            log.AddLog("You stepped on an unmoving slime. It hurt a bit.");
